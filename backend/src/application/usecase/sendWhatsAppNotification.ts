...
export default class sendWhatsAppNotification {
    ...
    public async execute(input: sendWhatsAppNotificationInput): Promise<Result<sendWhatsAppNotificationOutput>> {
        ...
        if (!appointment) return Result.fail('Não foi possível enviar a notificação. Agendamento não encontrado');
        if (!professional) return Result.fail('Não foi possível enviar a notificação. Profissional não encontrado');
        if (!client) return Result.fail('Não foi possível enviar a notificação. Cliente não encontrado');
        const send = await this.notification.send(client.cellPhone.DDD, client.cellPhone.number, MSG);

        return Result.ok<sendWhatsAppNotificationOutput>({result: true, msg: 'Notificação enviada com sucesso'});
    }
