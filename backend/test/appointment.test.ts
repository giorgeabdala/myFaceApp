...
beforeEach(() => {
    professional =  Professional.create('João', '11', '999999999', 'giorgeabdala@gmail.com');
    client =  Client.create('Maria', '11', '999999999', 'giorgeabdala@gmail.com');
...
describe('Deve testar a criação de agendamentos', () => {
    it('Deve criar um agendamento válido', () => {// adiciona uma hora
        const appointment = Appointment.create(startDate, endDate, 100, professional, client, status);
    ...
    it('Deve lançar um erro ao criar um agendamento com data de início inválida', () => {
        expect(() => Appointment.create(new Date('01/01/2022'), new Date(), 100, professional, client, status)).toThrow();
    } );
    it('Deve lançar um erro ao criar um agendamento com data de fim inválida', () => {
        expect(() => Appointment.create(new Date(), new Date(''), 100, professional, client, status)).toThrow();
    } );
    ...
    it('Deve lançar um erro ao criar um agendamento com profissional inválido', () => {
        expect(() => Appointment.create(new Date(), new Date(), 100, Professional.create('', '11', '999999999', 'giorgeabdala@gmail.com'), client, status)).toThrow();
} );
    it('Deve lançar um erro ao criar um agendamento com cliente inválido', () => {
        expect(() => Appointment.create(new Date(), new Date(), 100, professional,  Client.create('', '11', '999999999', 'giorgeabdala@gmail.com'), status)).toThrow();
    } );
    it ('Deve lançar um erro ao criar um agendamento com data de início maior que a data de fim', () => {
        expect(() => Appointment.create(endDate, startDate, 100, professional, client, status)).toThrow();
    } );
